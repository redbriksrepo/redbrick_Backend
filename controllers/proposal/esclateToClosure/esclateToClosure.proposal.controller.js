...
const updateFinalOfferAmmount = (req, res, next) => {
    try {
        ...
        Proposal.findById(Id).then((proposal) => {
            Proposal.updateOne({ _id: proposal._id }, { $set: { previousFinalOfferAmmount: proposal.finalOfferAmmount, clientFinalOfferAmmount: data.clientFinalOfferAmmount || proposal.finalOfferAmmount, escalateForCloser: true, status: 'Completed but not approved' } }).then((updateData) => {
